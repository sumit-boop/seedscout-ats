...
create policy "Members can manage candidates"
on public.candidates for all
using (
  exists (
    select 1 from public.company_members cm
    where cm.company_id = candidates.company_id and cm.user_id = auth.uid()
  )
)
with check (
  exists (
...
using (
  exists (
    select 1 from public.company_members cm
    where cm.company_id = candidate_notes.company_id and cm.user_id = auth.uid()
  )
)
with check (
  exists (
...
using (
  exists (
    select 1 from public.company_members cm
    where cm.company_id = interviews.company_id and cm.user_id = auth.uid()
  )
)
with check (
  exists (
