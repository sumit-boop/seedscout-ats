...
import { BriefcaseBusiness, ArrowDownToLine, Plus, X, Search, ChevronDown, Upload, User, Lock, LogOut, Settings, Users } from "lucide-react";
...
interface Client { username: string; password: string; companyName: string; }
interface AppState { jobs: Job[]; candidates: Candidate[]; ownerPool: string[]; }

/* ── Auth Config ── */
const CLIENTS: Client[] = [
  { username: "admin", password: "hiring2024", companyName: "Hiring Command Center" },
  { username: "client1", password: "client123", companyName: "Hiring Command Center" },
  ];

/* ── Storage ── */
const STORAGE_KEY = "hiring_command_center_v2";
...
function saveState(username: string, state: AppState) {
    ...
    localStorage.setItem(STORAGE_KEY + "_" + username, JSON.stringify(state));
}

/* ── AI Match ── */
const STOP_WORDS = new Set(["and","the","for","with","from","this","that","into","role","work","own","build","lead","close","candidate","experience","will","have","team","our","your","you","are","its","they","their","we","is","in","of","a","to","be","as","at","by","on","an","or","but","not","also","more","has","was","can","all","who","able","strong","high","fast"]);
function tokenize(text: string): string[] {
    return [...new Set(text.toLowerCase().replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(w => w.length > 2 && !STOP_WORDS.has(w)))].slice(0, 40);
}
function computeMatch(candidate: Candidate, job: Job | undefined): MatchResult {
    ...
    const candidateTokens = new Set(tokenize([candidate.role, candidate.cvText, candidate.notes, candidate.motivation, candidate.source].filter(Boolean).join(" ")));
    if (!jobTokens.length || !candidateTokens.size) return { score: 0, reason: "Add job description and CV text.", matched: [], missing: jobTokens.slice(0,4) };
    const matched = jobTokens.filter(w => candidateTokens.has(w));
    const missing = jobTokens.filter(w => !candidateTokens.has(w)).slice(0,4);
...
async function parseCV(file: File): Promise<string> {
    return new Promise((resolve) => {
          ...
          reader.onload = (e) => {
                  ...
                  resolve(cleaned || "CV uploaded – please add key skills manually.");
          };
          reader.onerror = () => resolve("CV uploaded – please add key skills manually.");
          if (file.type === "text/plain" || file.name.endsWith(".txt")) {
                  reader.readAsText(file);
          } else {
                  resolve(`CV file uploaded: ${file.name}. Please add key skills and experience in the CV notes field.`);
          }
    });
}
          ...
          <div className="login-screen">
                <div className="login-card">
                        <div className="login-logo">
                                  <BriefcaseBusiness size={40} color="var(--accent)" />
                        </div>div>
...
/* ── Manage Owners Modal ── */
function ManageOwnersModal({ ownerPool, onSave, onClose }: { ownerPool: string[]; onSave: (owners: string[]) => void; onClose: () => void }) {
    ...
    const removeOwner = (name: string) => setOwners(owners.filter(o => o !== name));
  
    return (
          <Modal title="Manage Owner Pool" onClose={onClose}>
                <div className="modal-form">
                        <p style={{ color: "var(--muted)", fontSize: "0.85rem", marginBottom: 4 }}>Add or remove people from the owner pool. These names will appear in role and candidate dropdowns.</p>p>
                        <div className="owner-list">
                          {owners.map(o => (
                        <div key={o} className="owner-tag">
                                      <span>{o}</span>span>
                                      <button onClick={() => removeOwner(o)} className="owner-remove"><X size={14} /></button>button>
                        </div>div>
                      ))}
                        </div>div>
                        <div className="owner-add-row">
                                  <input value={newOwner} onChange={e => setNewOwner(e.target.value)} onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addOwner())} placeholder="Add a name..." />
                                  <button type="button" className="ghost-button" onClick={addOwner}><Plus size={16} />Add</button>button>
                        </div>div>
                        <div className="modal-actions">
                                  <button type="button" className="ghost-button" onClick={onClose}>Cancel</button>button>
                                  <button type="button" className="primary-button" onClick={() => { onSave(owners); onClose(); }}><Plus size={16} />Save</button>button>
                        </div>div>
                </div>div>
          </Modal>Modal>
        );
...
/* ── Add Role Modal ── */
function AddRoleModal({ ownerPool, onAdd, onClose }: { ownerPool: string[]; onAdd: (job: Job) => void; onClose: () => void }) {
    ...
    return (
          <Modal title="Add new role" onClose={onClose}>
                <form onSubmit={e => { e.preventDefault(); if (form.title.trim()) { onAdd({ id: "job-" + form.title.toLowerCase().replace(/\s+/g,"-") + "-" + Date.now(), companyId: "client", status: "Active", targetDate: "", ...form }); onClose(); }}} className="modal-form">
                        ...
                        <div className="form-row">
                                  <label>Location<input value={form.location} onChange={e => set("location", e.target.value)} placeholder="e.g. Remote, Hybrid, NYC" /></label>label>
                        ...
                        <div className="form-row">
                                  <label>
                                              ...
                                              <select value={form.owner} onChange={e => set("owner", e.target.value)}>
                                                {ownerPool.map(o => <option key={o} value={o}>{o}</option>option>)}
                                              </select>select>
                                  </label>label>
                                  <label>
                                              Priority
                        ...
                        <label>Selling points<textarea rows={2} value={form.sellingPoints} onChange={e => set("sellingPoints", e.target.value)} placeholder="Why a candidate should join..." /></label>label>
                        <div className="modal-actions">
                                  <button type="button" className="ghost-button" onClick={onClose}>Cancel</button>button>
...
function AddCandidateModal({ jobs, ownerPool, onAdd, onClose }: { jobs: Job[]; ownerPool: string[]; onAdd: (c: Candidate) => void; onClose: () => void }) {
    ...
    return (
          <Modal title="Add candidate" onClose={onClose}>
                <form onSubmit={e => { e.preventDefault(); if (form.name.trim()) { onAdd({ id: "candidate-" + form.name.toLowerCase().replace(/\s+/g,"-") + "-" + Date.now(), companyId: "client", stage: "sourced", risk: "Low", motivation: "", phone: "", ...form }); onClose(); }}} className="modal-form">
                        <div className="form-row">
                        ...
                        <div className="form-row">
                                  ...
                                  <label>
                                              ...
                                              <select value={form.jobId} onChange={e => set("jobId", e.target.value)}>
                                                {jobs.map(j => <option key={j.id} value={j.id}>{j.title}</option>option>)}
                                              </select>select>
                                  </label>label>
                        </div>div>
                        <div className="form-row">
                                  <label>
                                              ...
                                              <select value={form.source} onChange={e => set("source", e.target.value)}>
                                                {["LinkedIn","Referral","AngelList","Inbound","Cold outreach","Other"].map(s => <option key={s}>{s}</option>option>)}
                                              </select>select>
                                  </label>label>
                                  <label>
                                              Priority
                        ...
                        <div className="cv-upload-section">
                                  ...
                                  <div className="cv-upload-row">
                                    ...
                                    {form.cvFileName && <span className="cv-filename">{form.cvFileName}</span>span>}
                                              <input ref={fileRef} type="file" accept=".txt,.pdf,.doc,.docx" style={{ display: "none" }} onChange={handleFileUpload} />
                                  </div>div>
                          {isParsing && <p className="cv-parsing-note">Extracting text from CV...</p>p>}
                        </div>div>
                        <label>
                                  CV / Skills notes
                                  <textarea rows={3} value={form.cvText} onChange={e => set("cvText", e.target.value)} placeholder="Key skills and experience (auto-filled from CV upload, or paste manually)..." />
                        </label>label>
                        <label>Notes<textarea rows={2} value={form.notes} onChange={e => set("notes", e.target.value)} placeholder="Any additional observations..." /></label>label>
                        <div className="modal-actions">
                                  <button type="button" className="ghost-button" onClick={onClose}>Cancel</button>button>
...
function ExportModal({ candidates, jobs, onClose }: { candidates: Candidate[]; jobs: Job[]; onClose: () => void }) {
    const csv = ["Name,Role,Stage,Priority,Source,Job,Match,Owner,Follow-up,Next Step,Notes",
                     ...candidates.map(c => {
    ...
    return (
          <Modal title="Export candidates" onClose={onClose}>
                <div className="modal-form">
                        <p style={{ color: "var(--muted)", marginBottom: 16 }}>{candidates.length} candidate{candidates.length !== 1 ? "s" : ""} will be exported as CSV.</p>p>
                        <div className="export-preview"><pre>{csv.substring(0, 400)}{csv.length > 400 ? "\n..." : ""}</pre>pre></div>div>
                        <div className="modal-actions">
                                  <button type="button" className="ghost-button" onClick={onClose}>Cancel</button>button>
                                  <button type="button" className="primary-button" onClick={() => { const b = new Blob([csv],{type:"text/csv"}); const u = URL.createObjectURL(b); const a = document.createElement("a"); a.href=u; a.download="candidates-export.csv"; a.click(); URL.revokeObjectURL(u); }}>
...
function Metric({ label, value, color }: { label: string; value: number; color?: string }) {
    return (
          <article className="metric">
                <span className="metric-label">{label}</span>span>
                <strong className={["metric-value", color ? `metric-${color}` : ""].join(" ").trim()}>{value}</strong>strong>
          </article>article>
        );
}
...
export default function HiringCommandCenter() {
    ...
    const handleLogout = () => {
          setLoggedIn(false);
          setCurrentUser("");
          setAppState({ jobs: [], candidates: [], ownerPool: ["Sam","Priya","Alex","Jordan","Taylor"] });
          setSearch("");
          setJobFilter("all");
    };
    ...
    const updateState = useCallback((updater: (prev: AppState) => AppState) => {
          setAppState(prev => {
                  ...
                  saveState(currentUser, next);
                  return next;
          });
    }, [currentUser]);
  
    const filteredCandidates = useMemo(() => appState.candidates.filter(c => {
    ...
    const active = filteredCandidates.filter(c => c.stage !== "hired");
    ...
    const dueSoon = active.filter(c => {
          ...
          const days = Math.round((new Date(c.followUpDate + "T00:00:00").getTime() - now.getTime()) / 86400000);
          ...
          <main className="app-shell">
            ...
            {modal === "owners" && <ManageOwnersModal ownerPool={appState.ownerPool} onSave={owners => updateState(s => ({ ...s, ownerPool: owners }))} onClose={() => setModal(null)} />}
                ...
                <section className="metrics" aria-label="Hiring metrics">
                        ...
                        <Metric label="High priority" value={highPriority} color="amber" />
                        <Metric label="Due in 3 days" value={dueSoon} color="rose" />
                        <Metric label="Offers out" value={offersOut} color="accent" />
                </section>section>
                  ...
                  {appState.jobs.length === 0 ? (
                      ...
                      <div className="jobs-grid">
                        {appState.jobs.map(job => {
                                      const jobCandidates = appState.candidates.filter(c => c.jobId === job.id);
                                      const topMatch = jobCandidates.map(c => ({ c, m: computeMatch(c, job) })).sort((a,b) => b.m.score - a.m.score)[0];
                                      return (
                                                        <article key={job.id} className="job-card">
                                                                          <header>
                                                                                              <div>
                                                                                                                    <h3>{job.title}</h3>h3>
                                                                                                                    <p className="job-meta-sub">{job.department} · {job.location}</p>p>
                                                                                                </div>div>
                                                                                              <span className={`job-status ${job.status.toLowerCase()}`}>{job.status}</span>span>
                                                                          </header>header>
                                                                          <div className="job-stats">
                                                                                              <div className="job-stat"><span>Pipeline</span>span><strong>{jobCandidates.length} candidates</strong>strong></div>div>
                                                                          ...
                                                                          <div className="ai-match-box">
                                                                            {topMatch ? (
                                                                                <>
                                                                                                        ...
                                                                                                        <span className="job-top-match">🏆 Top match: {topMatch.c.name} ({topMatch.m.score}%)</span>span>
                                                                                                        <span className="match-detail">{topMatch.m.reason}</span>span>
                                                                                  </>>
                                                                              ) : (
                ...
                <section className="command-center" aria-label="This week in hiring">
                        <SectionHeader eyebrow="Founder brief" title="This week in hiring" summary={`${active.length} active · ${highPriority} high priority · ${dueSoon} due soon`} />
                        <div className="command-grid">
                            ...
                            return (
                                                ...
                                                {filteredCandidates.length === 0 && <p style={{color:"var(--muted)",fontSize:"0.85rem",padding:"8px 0"}}>Add candidates to see match scores.</p>p>}
                                              </div>div>
                                  </article>article>
                                  <article className="command-panel">
                            ...
                            <div key={c.id} className="closing-item">
                                              ...
                                              <p>{c.motivation || "Keep the close plan current."}</p>p>
                            </div>div>
                          ))}
                                                {active.filter(c => c.stage === "offer" || c.risk !== "Low").length === 0 && <p style={{color:"var(--muted)",fontSize:"0.85rem",padding:"8px 0"}}>No high-risk candidates right now.</p>p>}
                                              </div>div>
                                  </article>article>
                        </div>div>
                </section>section>
                  ...
                  {STAGES.map(stage => (
                      <article key={stage.id} className="column">
                                  <header className="column-header">
                                                ...
                                                <span className="count">{filteredCandidates.filter(c => c.stage === stage.id).length}</span>span>
                                  </header>header>
                                  <div className="dropzone">
                                    {filteredCandidates.filter(c => c.stage === stage.id).map(c => {
                                        const job = appState.jobs.find(j => j.id === c.jobId);
                                        const m = computeMatch(c, job);
                                        const dateLabel = c.followUpDate ? new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric" }).format(new Date(c.followUpDate + "T00:00:00")) : "No date";
                                        return (
                                                            <article key={c.id} className="candidate-card">
                                                                                ...
                                                                                <dl>
                                                                                                      ...
                                                                                                      <div><dt>Due</dt>dt><dd>{dateLabel}</dd>dd></div>div>
                                                              ...
                                                              {c.cvFileName && <div className="cv-badge"><Upload size={11} />{c.cvFileName}</div>div>}
                                                                                <footer>
                                                                                                      ...
                                                                                                      <select value={c.stage} onChange={e => updateState(s => ({ ...s, candidates: s.candidates.map(x => x.id === c.id ? { ...x, stage: e.target.value } : x) }))}>
